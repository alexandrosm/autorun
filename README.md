...
- Native modal dialogs keep background controls inert, support Escape and retain voice-note discard confirmation. Screen transitions restore the top/focus without stealing capture focus. History rows work with keyboard and touch; destructive removal is separated from primary actions.
- Browser verification exercised GPS denial/wait/cancel and fresh-fix countdown, Stop/Resume and text notes, microphone/camera denial exits, a real AudioWorklet/MediaRecorder with synthetic sound, confirmed and audio-only pulse saves, injected storage failure and successful retry, actual export downloads, saved-history navigation, cancelled deletion and reload recovery. Layout checks covered widths from 320 to 1024 px, including 780 × 390 landscape. Browsing/expanding history after sync did not create new detail chunks. Synthetic signals prove the workflow, not physiological accuracy.
- No coach protocol, measurement algorithm, export schema or sync transport change. Existing saved data and queued audio/details are preserved. New exports now stamp the same application release version as the header.
...
- Direct-fetch timing cannot distinguish a denied browser permission from a network failure. **Sync to lab → Open lab page to finish sync** now offers an explicit handover after either a slow refusal or a timeout, without navigating to an unverified host automatically. Successful handover continuations remain automatic and limited to the original batch.
- Returning to an already-open app applies `#labsync` receipts on hash changes, page restoration and visibility return. Unmatched/expired receipts leave the queue intact. Export shows sync status and the same handover link as Home.
- Run/note uploads require `{ok: true, run_id}` or `{ok: true, note_id}` for the exact submitted identity; session receipts retain their session/chunk identity pair. Sync counts only durable local acknowledgements. A failed upload or local receipt stops the direct batch rather than waiting through every remaining item.
- The matching bridge atomically publishes complete run/audio files; failed publication preserves prior evidence. Its receiver bounds uploads to 15 seconds and protocol retrieval to 5 seconds, including response bodies. Malformed items cannot prevent later valid items from being processed; a transport failure returns completed receipts and leaves the remainder for retry. Update/restart the bridge with the app so strict acknowledgements are available.
- Session flushes snapshot pending writes rather than following new arrivals. Failed crash checkpoints retry when storage recovers; a failed checkpoint retirement remains counted and retryable. Stored key/chunk identity mismatches are retained rather than sent under another record's identity. Large saved handover snapshots can reload without the former unmatched 25,000-ID cutoff.
- Verification used isolated browser/storage/network failures, exact run/audio/session delivery, and four HTTPS-to-HTTP trips carrying 900 KB of audio while newly generated logs stayed queued. This does not establish which failure occurred on the phone; retry its existing queue after updating. Do not clear browser storage to repair sync.
...
Use **Sync to lab** on home Wi-Fi. Direct upload is attempted first; if direct access is unavailable, tap **Open lab page to finish sync**. A failed fetch alone cannot establish whether the host is down or the browser denied access. Each explicit handover snapshots its queue so interactions generated by the transfer itself wait for a later sync. Only exact, durably applied acknowledgements retire queued data. Unacknowledged items remain available for retry; do not clear browser storage. The lab bridge is LAN-only; do not expose its unauthenticated upload routes to the internet.
